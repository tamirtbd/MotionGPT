...
humanml3d_joints = ['root', 'RH', 'LH', 'BP', 'RK', 'LK', 'BT', 'RMrot', 'LMrot', 'BLN', 'RF', 'LF', 'BMN', 'RSI',
                    'LSI', 'BUN', 'RS', 'LS', 'RE', 'LE', 'RW', 'LW']
...
bpy.data.scenes["Scene"].frame_end = len(a)

# Select SMPLX skeleton to apply KF animation
C = bpy.context
bpy.ops.object.select_all(action='DESELECT')
skel = bpy.data.objects['SMPLX']
...
bpy.ops.transform.translate(value=(-0, -0, -75.5), orient_type='GLOBAL',
                            orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL',
...
# Retarget To Miaxmo
bpy.ops.object.select_all(action='DESELECT')
skel_target = bpy.data.objects['mixamorig_Root']
skel_target.select_set(True)
bpy.context.view_layer.objects.active = skel_target

ctx = skel_target.retargeting_context
drivers = sys.modules['blender-animation-retargeting-stable.drivers']
ctx.setting_disable_drivers = False
drivers.update_drivers(ctx)

baking = sys.modules['blender-animation-retargeting-stable.baking']
baking.transfer_anim(ctx)

# Export USD
export_path = args['export_path']

bpy.ops.wm.usd_export(
    filepath=export_path, export_animation=True, export_normals=False,
    export_materials=False, export_shapekeys=True
)
